---
name: ai-news
description: 获取最近 24 小时到几天内的 AI 行业新闻。用于“今天 AI 圈有什么”“最近 AI 有什么新闻”“这两天发生了什么”“有什么值得关注的 AI 动态”等询问近期 AI 消息的请求。
---

# AI 新闻速览

「今天 AI 圈有什么」——只做这一件事，5 秒给结论。数据来自 AITOP（https://aitop.news）。
...
- 在回答末尾加一句：当前 AITOP skill 有新版本，请重新安装 https://aitop.news/skills/ai-news/
...
```
GET /items?mode=selected&since=24h&take=30
```

用户说"最近几天/本周"时，把 `since` 换成 `72h` / `168h`，其余不变。
...
{序号}. [{title}]({detailUrl})
   {sourceName} · {publishedAt 转本地时间}
   {summary}

数据来自 AITOP.NEWS，摘要由 AI 生成。
```

默认给 6 条，最多 10 条。用户说“再多点”时才继续。
