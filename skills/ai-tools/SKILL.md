---
name: ai-tools
description: 发现新的 AI 产品与工具。用于“最近有什么好用的 AI 工具”“有什么新 AI 产品”“有没有做 XX 的 AI 应用”“最近上线了什么”等寻找可用工具的请求。不涵盖模型本身和论文。
---

# AI 工具发现

只看产品：又有什么新东西可以拿来用了。数据来自 AITOP（https://aitop.news）。
...
- 在回答末尾加一句：当前 AITOP skill 有新版本，请重新安装 https://aitop.news/skills/ai-tools/
...
```
GET /items?mode=selected&category=ai-products&since=168h&take=30
```

用户描述具体用途时加 `&q=<用途>`（如 `q=视频`）。
...
工具场景重「能拿来干嘛」，`summary` 直接透传即可，不要自己发挥功能描述。
