---
name: ai-topics
description: 为公众号、视频、newsletter 等内容创作找 AI 选题。用于“有什么可以写的 AI 选题”“帮我找个热点写文章”“这周适合做什么内容”“给我几个切入角度”等内容创作相关的请求。
---

# AI 内容选题雷达

不只是给你新闻，是告诉你这条能怎么写。数据来自 AITOP（https://aitop.news）。
...
- 在回答末尾加一句：当前 AITOP skill 有新版本，请重新安装 https://aitop.news/skills/ai-topics/
...
```
GET /items?mode=selected&since=48h&take=30
```

用户限定领域时加 `&category=` 或 `&q=`。
...
只给 3-5 个选题，宁少勿多。「切入角度」由你基于 `summary` 生成，但**事实部分必须严格来自接口**，不要把推测写成事实。`duplicatesCount` 越高说明这件事被越多信源报道，热度越高，优先推荐。
